...
/* ── AJAX : contenu d'un événement agenda pour la popup ── */
add_action( 'wp_ajax_berre_ev_content',        'berre_ev_content_ajax' );
...
function berre_cal_render_grid( $year, $mon ) {
    ...
    $by_day = [];
    foreach ($posts as $post) {
        $s = get_post_meta($post->ID,'berre_event_date_start',true) ?: get_the_date('Y-m-d',$post);
        $e = get_post_meta($post->ID,'berre_event_date_end',true) ?: $s;
        $time = get_post_meta($post->ID,'berre_event_time',true);
    ...
    $first_dow = (int)(new DateTime("$year-$mon-01"))->format('N') - 1;
    $days_in   = (int)(new DateTime("$year-$mon-01"))->format('t');
    $prev_last = (int)(new DateTime("$year-$mon-01"))->modify('-1 day')->format('t');
    $today_d   = (int)date('j'); $today_m = (int)date('n'); $today_y = (int)date('Y');
    $html = '';
    for ($i=$first_dow-1;$i>=0;$i--)
        $html .= '<div class="berre-cal__day berre-cal__day--other"><span class="berre-cal__day-num">'.($prev_last-$i).'</span></div>';
    for ($d=1;$d<=$days_in;$d++) {
        $is_t = ($d===$today_d && $mon===$today_m && $year===$today_y);
        $evs  = $by_day[$d] ?? [];
    ...
    $last_dow = (int)(new DateTime("$year-$mon-$days_in"))->format('N') - 1;
...
/* ── Endpoint AJAX pour la navigation ── */
add_action('wp_ajax_berre_cal_nav',        'berre_cal_ajax_nav');
add_action('wp_ajax_nopriv_berre_cal_nav', 'berre_cal_ajax_nav');
function berre_cal_ajax_nav() {
    $month = sanitize_text_field($_GET['month'] ?? date('Y-m'));
    if (!preg_match('/^\d{4}-\d{2}$/',$month)) $month = date('Y-m');
    list($y,$m) = explode('-',$month);
    $year = intval($y); $mon = intval($m);
    $months_fr = ['','Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'];
    wp_send_json([
        'label' => $months_fr[$mon].' '.$year,
        'html'  => berre_cal_render_grid($year,$mon),
...
add_shortcode( 'berre_calendrier_agenda', function() {
    $raw = isset($_GET['cal_month']) ? sanitize_text_field($_GET['cal_month']) : date('Y-m');
    if (!preg_match('/^\d{4}-\d{2}$/',$raw)) $raw = date('Y-m');
    list($y,$m) = explode('-',$raw);
    $year = intval($y); $mon = intval($m);
    $months_fr = ['','Janvier','Février','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','Décembre'];
    $prev = date('Y-m', mktime(0,0,0,$mon-1,1,$year));
    $next = date('Y-m', mktime(0,0,0,$mon+1,1,$year));
...
/* ── Shortcode [berre_event_meta] — dates, heures et lieu d'un événement ── */
add_shortcode( 'berre_event_meta', function() {
    $pid        = get_the_ID();
    $date_start = get_post_meta( $pid, 'berre_event_date_start', true );
    $date_end   = get_post_meta( $pid, 'berre_event_date_end',   true );
    $time       = get_post_meta( $pid, 'berre_event_time',       true );
    $location   = get_post_meta( $pid, 'berre_event_location',   true );

    if ( ! $date_start && ! $date_end && ! $time && ! $location ) return '';

    $mfr = ['','janvier','février','mars','avril','mai','juin',
             'juillet','août','septembre','octobre','novembre','décembre'];
    $dfr = ['dimanche','lundi','mardi','mercredi','jeudi','vendredi','samedi'];

    $fmt = function($ymd) use ($mfr, $dfr) {
        if (!$ymd) return '';
        $ts = strtotime($ymd);
        return $dfr[date('w',$ts)] . ' ' . (int)date('j',$ts) . ' ' . $mfr[(int)date('n',$ts)] . ' ' . date('Y',$ts);
    };

    $s = $fmt($date_start);
    $e = ($date_end && $date_end !== $date_start) ? $fmt($date_end) : '';

    ob_start(); ?>
    <div class="berre-event-meta">
        <?php if ($s): ?>
        <div class="berre-event-meta__row">
            <span class="berre-event-meta__icon"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></span>
            <span class="berre-event-meta__text"><?php echo $e ? 'Du ' . esc_html($s) . ' au ' . esc_html($e) : esc_html(ucfirst($s)); ?></span>
        </div>
        <?php endif; if ($time): ?>
        <div class="berre-event-meta__row">
            <span class="berre-event-meta__icon"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg></span>
            <span class="berre-event-meta__text"><?php echo esc_html($time); ?></span>
        </div>
        <?php endif; if ($location): ?>
        <div class="berre-event-meta__row">
            <span class="berre-event-meta__icon"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg></span>
            <span class="berre-event-meta__text"><?php echo esc_html($location); ?></span>
        </div>
        <?php endif; ?>
    </div>
    <?php
