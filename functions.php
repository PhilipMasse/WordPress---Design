...
/* ============================================================
   COMMUNE — Page d'administration dédiée
   Option WP : berre_commune  (priorité sur berre_page_content)
   ============================================================ */
...
function berre_get_commune() {
    $saved = get_option('berre_commune');
    if ( ! empty($saved) ) return array_merge(berre_commune_defaults(), $saved);
    // Fallback sur berre_page_content
    $pc = berre_get_page_content();
...
add_action('admin_init', function() {
    ...
    $d = [];
    foreach (['eyebrow','title','description','stat1_val','stat1_lbl','stat2_val','stat2_lbl',
              'stat3_val','stat3_lbl','stat4_val','stat4_lbl','btn_text','btn_url'] as $k) {
        $d[$k] = sanitize_text_field($_POST['commune_'.$k] ?? '');
    }
    $d['description'] = sanitize_textarea_field($_POST['commune_description'] ?? '');
    $d['btn_url']     = esc_url_raw($_POST['commune_btn_url'] ?? '#');
    $d['btn_target']  = ($_POST['commune_btn_target'] ?? '_self') === '_blank' ? '_blank' : '_self';

    update_option('berre_commune', $d);
...
function berre_commune_admin_page() {
    ...
    $pages = berre_get_page_options();
    ?>
    <style>
    .berre-commune-wrap { max-width:780px; margin-top:20px; }
    .berre-commune-card { background:#fff;border:1px solid #ddd;border-radius:8px;overflow:hidden;margin-bottom:18px; }
    .berre-commune-card-head { background:#f6f7f7;border-bottom:1px solid #eee;padding:13px 20px;font-size:13px;font-weight:700;color:#333; }
    .berre-commune-card-body { padding:20px; }
    .berre-commune-field { margin-bottom:16px; }
    .berre-commune-field label { display:block;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#888;margin-bottom:5px; }
    .berre-commune-field input[type=text],.berre-commune-field textarea {
        width:100%;padding:8px 11px;border:1px solid #ddd;border-radius:5px;font-size:13px;font-family:inherit;
    }
    .berre-commune-field input:focus,.berre-commune-field textarea:focus {
        border-color:#2D6AB0;outline:none;box-shadow:0 0 0 2px rgba(45,106,176,.12);
    }
    .berre-commune-field textarea { resize:vertical;min-height:80px; }
    .berre-commune-stats-grid { display:grid;grid-template-columns:1fr 1fr;gap:12px; }
    .berre-commune-stat-item { border:1px solid #e8e8e8;border-radius:6px;padding:12px;background:#fafafa; }
    .berre-commune-stat-item .berre-commune-row { display:grid;grid-template-columns:1fr 1fr;gap:8px; }
    .berre-commune-row { display:grid;grid-template-columns:1fr 1fr;gap:12px; }
    /* Preview */
    .berre-commune-preview {
        background:linear-gradient(135deg,#2D6AB0,#1a3e72);
        border-radius:8px;padding:24px;color:#fff;margin-bottom:18px;
    }
    .berre-commune-preview__eyebrow { font-size:10px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:rgba(255,255,255,.5);margin-bottom:8px; }
    .berre-commune-preview__title { font-size:1.4rem;font-weight:800;line-height:1.25;margin-bottom:12px; }
    .berre-commune-preview__desc { font-size:12px;color:rgba(255,255,255,.75);line-height:1.6;margin-bottom:14px; }
    .berre-commune-preview__stats { display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:14px; }
    .berre-commune-preview__stat { background:rgba(255,255,255,.15);border-radius:6px;padding:10px 8px;text-align:center; }
    .berre-commune-preview__stat-val { font-size:1.1rem;font-weight:800;display:block; }
    .berre-commune-preview__stat-lbl { font-size:9px;text-transform:uppercase;letter-spacing:.08em;color:rgba(255,255,255,.6); }
    .berre-commune-preview__btn { display:inline-block;background:#DEA128;color:#1a3e72;padding:9px 20px;border-radius:20px;font-size:12px;font-weight:700;text-decoration:none; }
    </style>

    <div class="wrap berre-commune-wrap">
        <h1>🏘 Commune — Bloc page d'accueil</h1>
        <?php if ($saved): ?>
        <div class="notice notice-success is-dismissible"><p>✅ Sauvegardé avec succès.</p></div>
        <?php endif; ?>

        <!-- Aperçu temps réel -->
        <div class="berre-commune-preview">
            <div class="berre-commune-preview__eyebrow" id="prev-eyebrow"><?php echo esc_html($c['eyebrow']); ?></div>
            <div class="berre-commune-preview__title"   id="prev-title"><?php echo esc_html($c['title']); ?></div>
            <div class="berre-commune-preview__desc"    id="prev-desc"><?php echo esc_html($c['description']); ?></div>
            <div class="berre-commune-preview__stats">
                <?php foreach ([1,2,3,4] as $n): ?>
                <div class="berre-commune-preview__stat">
                    <span class="berre-commune-preview__stat-val" id="prev-stat<?php echo $n; ?>-val"><?php echo esc_html($c["stat{$n}_val"]); ?></span>
                    <span class="berre-commune-preview__stat-lbl" id="prev-stat<?php echo $n; ?>-lbl"><?php echo esc_html($c["stat{$n}_lbl"]); ?></span>
                </div>
                <?php endforeach; ?>
            </div>
            <span class="berre-commune-preview__btn" id="prev-btn"><?php echo esc_html($c['btn_text']); ?></span>
        </div>
        ...
        <form method="post" id="berre-commune-form">
            ...
            <div class="berre-commune-card">
                <div class="berre-commune-card-head">📝 Textes</div>
                <div class="berre-commune-card-body">
                    <div class="berre-commune-field">
                        <label>Surtitre <span style="color:#aaa;font-weight:400">(au-dessus du titre)</span></label>
                        <input type="text" name="commune_eyebrow" value="<?php echo esc_attr($c['eyebrow']); ?>"
                               data-preview="prev-eyebrow" placeholder="Découvrir la commune">
                    </div>
                    <div class="berre-commune-field">
                        <label>Titre principal</label>
                        <input type="text" name="commune_title" value="<?php echo esc_attr($c['title']); ?>"
                               data-preview="prev-title" placeholder="Un village d'exception…">
                    </div>
                    <div class="berre-commune-field">
                        <label>Description</label>
                        <textarea name="commune_description" data-preview="prev-desc"><?php echo esc_textarea($c['description']); ?></textarea>
                    </div>
                </div>
            </div>
            ...
            <!-- Chiffres clés -->
            <div class="berre-commune-card">
                <div class="berre-commune-card-head">📊 Chiffres clés</div>
                <div class="berre-commune-card-body">
                    <div class="berre-commune-stats-grid">
                        <?php foreach ([1,2,3,4] as $n): ?>
                        <div class="berre-commune-stat-item">
                            <div class="berre-commune-row">
                                <div class="berre-commune-field" style="margin:0">
                                    <label>Valeur <?php echo $n; ?></label>
                                    <input type="text" name="commune_stat<?php echo $n; ?>_val"
                                           value="<?php echo esc_attr($c["stat{$n}_val"]); ?>"
                                           data-preview="prev-stat<?php echo $n; ?>-val">
                                </div>
                                <div class="berre-commune-field" style="margin:0">
                                    <label>Libellé <?php echo $n; ?></label>
                                    <input type="text" name="commune_stat<?php echo $n; ?>_lbl"
                                           value="<?php echo esc_attr($c["stat{$n}_lbl"]); ?>"
                                           data-preview="prev-stat<?php echo $n; ?>-lbl">
                                </div>
                            </div>
                        </div>
                        <?php endforeach; ?>
            ...
            <!-- Bouton -->
            <div class="berre-commune-card">
                <div class="berre-commune-card-head">🔘 Bouton d'action</div>
                <div class="berre-commune-card-body">
                    <div class="berre-commune-field">
                        <label>Texte du bouton</label>
                        <input type="text" name="commune_btn_text" value="<?php echo esc_attr($c['btn_text']); ?>"
                               data-preview="prev-btn" placeholder="Explorer nos sentiers →">
                    </div>
                    <div class="berre-commune-field">
                        <label>Lien du bouton</label>
                        <?php echo berre_url_picker_html('commune_btn_url', $c['btn_url'] ?? '/decouvrir'); ?>
                    </div>
                    <div class="berre-commune-field">
                        <label>Ouverture</label>
                        <select name="commune_btn_target" style="padding:6px 10px;border:1px solid #ddd;border-radius:4px">
                            <option value="_self"  <?php selected($c['btn_target']??'_self', '_self'); ?>>Même page</option>
                            <option value="_blank" <?php selected($c['btn_target']??'_self', '_blank'); ?>>Nouvel onglet ↗</option>
                        </select>
                    </div>
                </div>
            </div>
    ...
    document.querySelectorAll('[data-preview]').forEach(function(el) {
        ['input','keyup','change'].forEach(function(evt) {
            el.addEventListener(evt, function() {
                var target = document.getElementById(this.dataset.preview);
